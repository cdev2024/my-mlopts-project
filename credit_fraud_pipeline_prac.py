...
for params in param_list:
    run_name = f"n{params['n_estimators']}_depth{params['max_depth']}_min{params['min_samples_split']}"
    # 파이프라인 설계---------------------------------------------
    pipe = Pipeline([
        ("preprocessor", preprocessor),   # 위에서 만든 전처리 서브파이프라인 사용
    ...
    # run_results에 담기----------------------------------------
    joblib.dump(pipe, f"model/pipeline_n{params['n_estimators']}_d{params['max_depth']}_m{params['min_samples_split']}.pkl")   
    run_results.append({
        "run_name": run_name,
        "accuracy": acc,
        "train_acc": train_acc,
        "model_uri": f"model/pipeline_n{params['n_estimators']}_d{params['max_depth']}_m{params['min_samples_split']}.pkl"
    }) 
